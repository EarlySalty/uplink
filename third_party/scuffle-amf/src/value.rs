...
impl Amf0Value<'_> {
    ...
    pub fn into_owned(self) -> Amf0Value<'static> {
        match self {
            ...
            Amf0Value::Object(v) => Amf0Value::Object(
                v.into_iter()
            ...
            Amf0Value::Array(v) => {
                Amf0Value::Array(v.into_owned().into_iter().map(|v| v.into_owned()).collect())
            }
        }
    }
...
mod tests {
    ...
    fn from() {
        ...
        let object: Amf0Object = [("a".into(), Amf0Value::Boolean(true))]
            .into_iter()
            .collect();
        ...
        assert_eq!(
            value,
            Amf0Value::Array(Cow::Owned(vec![Amf0Value::Boolean(true)]))
        );
    }
    ...
    fn unsupported_marker() {
        ...
        assert!(matches!(
            err,
            Amf0Error::UnsupportedMarker(Amf0Marker::MovieClipMarker)
        ));
    }
    ...
    fn array() {
        ...
        assert_eq!(
            value,
            Amf0Value::Array(Cow::Borrowed(&[Amf0Value::Boolean(true)]))
        );

        let mut serialized = vec![];
        value
            .encode(&mut Amf0Encoder::new(&mut serialized))
            .unwrap();
    ...
    fn deserialize() {
        ...
        impl serde::de::Error for TestError {
            fn custom<T: std::fmt::Display>(msg: T) -> Self {
                assert_eq!(
                    msg.to_string(),
                    "invalid type: Option value, expected a byte slice"
                );
                Self
            }
        }
                ...
                T: serde::de::DeserializeSeed<'de>,
            {
                match self.mode {
                    Mode::Seq => {
                        self.mode = Mode::End;
                        Ok(Some(
                            seed.deserialize(TestDeserializer { mode: Mode::I64 })?,
                        ))
                    }
                    Mode::End => Ok(None),
                    _ => Err(TestError),
        ...
        impl<'de> MapAccess<'de> for TestDeserializer {
                ...
                K: serde::de::DeserializeSeed<'de>,
            {
                match self.mode {
                    Mode::Map => Ok(Some(
                        seed.deserialize(TestDeserializer { mode: Mode::Str })?,
                    )),
                    Mode::End => Ok(None),
                    _ => Err(TestError),
                }
        ...
        test_de(
            Mode::Seq,
            Amf0Value::Array(Cow::Owned(vec![Amf0Value::Number(1.0)])),
        );
        test_de(
            Mode::Map,
            Amf0Value::Object(
                [("hello".into(), Amf0Value::Number(1.0))]
                    .into_iter()
                    .collect(),
            ),
        );
...
// Modified by Uplink: formatting only; see ../PATCHES.md.
