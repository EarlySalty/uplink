...
mod tests {
    ...
    fn command_result_level_to_str() {
        ...
        assert_eq!(
            CommandResultLevel::Unknown("custom".to_string()).as_ref(),
            "custom"
        );
    }
    ...
    fn command_result_level_into_string() {
        ...
        assert_eq!(
            CommandResultLevel::Unknown("custom".to_string()).to_string(),
            "custom"
        );
    }
    ...
    fn netstream_command_write() {
        ...
        assert!(matches!(
            err,
            RtmpError::Command(CommandError::NoClientImplementation)
        ));
    }
}
