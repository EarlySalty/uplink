...
impl Command<'_> {
    ...
    pub fn read(payload: Bytes) -> Result<Self, CommandError> {
        Self::read_with_limits(payload, scuffle_amf0::DecodeLimits::default())
    }

    /// Reads a command with budgets applied before AMF allocations.
    pub fn read_with_limits(
        payload: Bytes,
        limits: scuffle_amf0::DecodeLimits,
    ) -> Result<Self, CommandError> {
        let mut decoder = Amf0Decoder::from_buf_with_limits(payload, limits);
        ...
        // Some recognized commands leave optional AMF arguments unread.
        // Validate and account for those too before accepting the message.
        decoder.decode_all()?;
        decoder.finish()?;
...
impl<'a> CommandType<'a> {
    fn read(
        ...
        Ok(Self::Unknown(UnknownCommand {
            command_name,
            values,
        }))
    }
...
mod tests {
    ...
    fn test_command_result_level() {
        assert_eq!(
            "warning".parse::<CommandResultLevel>().unwrap(),
            CommandResultLevel::Warning
        );
        assert_eq!(
            "status".parse::<CommandResultLevel>().unwrap(),
            CommandResultLevel::Status
        );
        assert_eq!(
            "error".parse::<CommandResultLevel>().unwrap(),
            CommandResultLevel::Error
        );
        assert_eq!(
