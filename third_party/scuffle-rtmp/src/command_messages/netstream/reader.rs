...
mod tests {
    ...
    use crate::command_messages::netstream::{
        NetStreamCommand, NetStreamCommandPublishPublishingType,
    };

    #[test]
    fn test_command_no_payload() {
        let command =
            NetStreamCommand::read("closeStream", &mut Amf0Decoder::from_buf(Bytes::new()))
                .unwrap()
                .unwrap();
    ...
    fn play_command() {
        ...
        let command = NetStreamCommand::read(
            "play",
            &mut Amf0Decoder::from_buf(Bytes::from_owner(payload)),
        )
        .unwrap()
        .unwrap();
    ...
    fn publish() {
        ...
        encoder
            .serialize(NetStreamCommandPublishPublishingType::Record)
            .unwrap();
