...
impl MessageData<'_> {
    ...
    pub fn read(chunk: &Chunk) -> Result<Self, crate::error::RtmpError> {
        Self::read_with_limits(chunk, scuffle_amf0::DecodeLimits::default())
    }

    /// Reads commands with AMF budgets; media payloads remain opaque.
    pub fn read_with_limits(
        chunk: &Chunk,
        limits: scuffle_amf0::DecodeLimits,
    ) -> Result<Self, crate::error::RtmpError> {
        match chunk.message_header.msg_type_id {
            ...
            MessageType::CommandAMF0 => Ok(Self::Amf0Command(Command::read_with_limits(
                chunk.payload.clone(),
                limits,
            )?)),
...
mod tests {
    ...
    fn test_parse_command() {
        ...
        let object: Amf0Object = [("app".into(), Amf0Value::String("testapp".into()))]
            .into_iter()
            .collect();
        ...
        match message {
            MessageData::Amf0Command(command) => {
                ...
                let CommandType::NetConnection(NetConnectionCommand::Connect(connect)) =
                    command_type
                else {
                    panic!("wrong command");
                };
    ...
    fn test_parse_metadata() {
        ...
        let object: Amf0Object = [("duration".into(), Amf0Value::Number(0.0))]
            .into_iter()
            .collect();
        encoder.encode_object(&object).unwrap();
