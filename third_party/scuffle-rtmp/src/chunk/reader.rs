...
/// Resource policy, separate from the peer's negotiated chunk framing size.
#[derive(Clone, Copy, Debug)]
pub struct ChunkReaderLimits {
    /// Maximum complete message payload in bytes.
    pub max_message_bytes: usize,
    /// Maximum AMF command/metadata message payload before accumulation.
    pub max_command_bytes: usize,
    /// Maximum remembered chunk stream IDs.
    pub max_chunk_streams: usize,
    /// Maximum concurrently incomplete messages.
    pub max_partial_messages: usize,
    /// Maximum sum of declared lengths reserved for incomplete messages.
    pub max_partial_bytes: usize,
    /// Largest peer chunk size accepted; exceeding it closes the connection.
    pub max_chunk_size: usize,
}

impl Default for ChunkReaderLimits {
    fn default() -> Self {
        Self {
            max_message_bytes: MAX_PARTIAL_CHUNK_SIZE,
            max_command_bytes: 64 * 1024,
            max_chunk_streams: MAX_PREVIOUS_CHUNK_HEADERS,
            max_partial_messages: MAX_PARTIAL_CHUNK_COUNT,
...
impl ChunkReaderLimits {
    /// Validate before creating parser state.
    pub fn validate(&self) -> Result<(), ChunkReadError> {
        if self.max_message_bytes == 0
            || self.max_message_bytes > 0xff_ffff
            || self.max_command_bytes == 0
            || self.max_command_bytes > self.max_message_bytes
            || self.max_chunk_streams == 0
            || self.max_chunk_streams > 65_599
            || self.max_partial_messages == 0
            || self.max_partial_messages > self.max_chunk_streams
            || self.max_partial_bytes == 0
            || !(INIT_CHUNK_SIZE..=MAX_CHUNK_SIZE).contains(&self.max_chunk_size)
        {
            return Err(ChunkReadError::InvalidLimits);
        }
        Ok(())
    }
}
...
pub struct ChunkReader {
    ...
    limits: ChunkReaderLimits,
    partial_reserved_bytes: usize,
    timestamp_deltas: HashMap<u32, u32>,
}

impl Default for ChunkReader {
    fn default() -> Self {
        Self::with_limits(ChunkReaderLimits::default()).expect("valid default limits")
    }
}

impl ChunkReader {
    /// Create a parser with validated limits. Maps start empty, not peer-sized.
    pub fn with_limits(limits: ChunkReaderLimits) -> Result<Self, ChunkReadError> {
        limits.validate()?;
        Ok(Self {
            ...
            limits,
            partial_reserved_bytes: 0,
            timestamp_deltas: HashMap::new(),
        })
    }
    ...
    pub fn read_chunk(
        &mut self,
        buffer: &mut BytesMut,
    ) -> Result<Option<Chunk>, crate::error::RtmpError> {
        ...
        loop {
            ...
            // An uncompressed continuation is allowed only when it repeats
            // the same logical message header. Never replace an active message.
            let continuing = self
                .partial_chunks
                .keys()
                .any(|key| key.0 == header.chunk_stream_id);
            if continuing && header.format != ChunkType::Type3 {
                let previous = &self.previous_chunk_headers[&header.chunk_stream_id];
                if message_header.msg_length != previous.msg_length
                    || message_header.msg_stream_id != previous.msg_stream_id
                    || message_header.msg_type_id != previous.msg_type_id
                    || message_header.timestamp != previous.timestamp
                {
                    return Err(ChunkReadError::IncompleteMessageReplaced.into());
                }
            }
            if !self
                .previous_chunk_headers
                .contains_key(&header.chunk_stream_id)
                && self.previous_chunk_headers.len() >= self.limits.max_chunk_streams
            {
                return Err(ChunkReadError::TooManyPreviousChunkHeaders.into());
            }
            let payload_limit = if matches!(
                message_header.msg_type_id,
                MessageType::CommandAMF0
                    | MessageType::DataAMF0
                    | MessageType::CommandAMF3
                    | MessageType::DataAMF3
                    | MessageType::SharedObjAMF0
                    | MessageType::SharedObjAMF3
            ) {
                self.limits.max_command_bytes
            } else {
                self.limits.max_message_bytes
            };
            if message_header.msg_length as usize > payload_limit {
                return Err(ChunkReadError::PartialChunkTooLarge(
                    message_header.msg_length as usize,
                )
                .into());
            }
            if !continuing && message_header.msg_length as usize > self.max_chunk_size {
                if self.partial_chunks.len() >= self.limits.max_partial_messages {
                    return Err(ChunkReadError::TooManyPartialChunks.into());
                }
                if message_header.msg_length as usize
                    > self.limits.max_partial_bytes - self.partial_reserved_bytes
                {
                    return Err(ChunkReadError::PartialBytesLimit.into());
                }
            }
            ...
            if count > self.limits.max_chunk_streams {
            ...
            if !continuing && header.format != ChunkType::Type3 {
                let delta = if header.format == ChunkType::Type0 {
                    message_header.timestamp
                } else {
                    message_header.timestamp.wrapping_sub(
                        self.previous_chunk_headers[&header.chunk_stream_id].timestamp,
                    )
                };
                self.timestamp_deltas.insert(header.chunk_stream_id, delta);
            }
            ...
            if payload.len() == message_header.msg_length as usize {
                ...
                let partial_chunk = match self.partial_chunks.get_mut(&key) {
                    ...
                    None => {
                        ...
                        let reserved = message_header.msg_length as usize;
                        if reserved > self.limits.max_partial_bytes - self.partial_reserved_bytes {
                            return Err(ChunkReadError::PartialBytesLimit.into());
                        }
                        self.partial_reserved_bytes += reserved;
                        self.partial_chunks
                            .insert(key, BytesMut::with_capacity(reserved));
                ...
                let length = {
                    ...
                    if payload.len() > self.limits.max_message_bytes - partial_chunk.len() {
                        return Err(crate::error::RtmpError::ChunkRead(
                            ChunkReadError::PartialChunkTooLarge(
                ...
                if length == message_header.msg_length as usize {
                    self.partial_reserved_bytes -= message_header.msg_length as usize;
                    return Ok(Some(Chunk {
                        basic_header: header,
                        message_header,
    ...
    ) -> Result<ChunkBasicHeader, Option<io::Error>> {
        ...
        match header.format {
            ...
            ChunkType::Type0 => {
                ...
                let msg_length = cursor
                    ...
                    .eof_to_none()
                    .map_err(|e| e.map(crate::error::RtmpError::Io))?;
                if msg_length as usize > self.limits.max_message_bytes {
            ...
            ChunkType::Type3 => {
                    ...
                    .any(|key| key.0 == header.chunk_stream_id)
                {
                    let delta = self
                        ...
                        .copied()
                        .unwrap_or(0);
                    previous_header.timestamp = previous_header.timestamp.wrapping_add(delta);
                }
                Ok(previous_header)
            }
        }
    ...
    fn get_payload_range(
        ...
        let remaining_read_length = (message_header.msg_length as usize)
            ...
            .ok_or(Some(crate::error::RtmpError::ChunkRead(
                ChunkReadError::IncompleteMessageReplaced,
            )))?;
...
mod tests {
    ...
    fn test_reader_extended_timestamp() {
        ...
        // This upstream vector changes timestamps inside a still incomplete message.
        // Reject it instead of merging distinct logical messages.
        assert!(unpacker.read_chunk(&mut buf).is_err());
    }
