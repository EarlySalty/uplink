...
use crate::command_messages::netconnection::{
    CapsExMask, NetConnectionCommand, NetConnectionCommandConnect,
    NetConnectionCommandConnectResult,
};
...
pub use limits::ServerSessionLimits;
use output::BoundedWriter;
...
impl<S, H> ServerSession<S, H> {
    ...
    pub fn new(io: S, handler: H) -> Self {
        Self {
            ...
            limits: ServerSessionLimits::default(),
            handshake_started: false,
            has_published: false,
        }
    }

    /// Configure resource limits before the connection starts processing.
    pub fn with_limits(mut self, limits: ServerSessionLimits) -> Result<Self, ServerSessionError> {
        limits.validate()?;
        self.chunk_reader = ChunkReader::with_limits(limits.chunk)
...
impl<S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin, H: SessionHandler>
    ServerSession<S, H>
{
    ...
    pub async fn run(mut self) -> Result<bool, crate::error::RtmpError> {
        ...
        let ctx = self
            .ctx
            .clone()
            .unwrap_or_else(scuffle_context::Context::global);
        let handshake_deadline = tokio::time::Instant::now() + self.limits.handshake_timeout;
        ...
        loop {
            let step = tokio::time::timeout_at(
                handshake_deadline,
                async {
                    let complete = self.drive_handshake(&mut handshaker).await?;
                    self.flush().await?;
                    Ok::<_, crate::error::RtmpError>(complete)
                }
                .with_context(&ctx),
            )
            .await
            ...
            match step {
                Some(Ok(false)) => {} // Continue driving
                Some(Ok(true)) => break,
        ...
        let connected_deadline = tokio::time::Instant::now() + self.limits.connect_timeout;
        let publish_deadline = tokio::time::Instant::now() + self.limits.publish_timeout;

        // Drive the session to completion
        loop {
            let result = if !self.has_published {
                let deadline = if self.app_name.is_none() {
                    connected_deadline.min(publish_deadline)
                } else {
                    publish_deadline
                };
                tokio::time::timeout_at(deadline, self.drive())
                    .await
                    .map_err(ServerSessionError::Timeout)?
            } else {
                self.drive().await
    ...
    async fn drive_handshake(
        ...
        let read_size = handshake::RTMP_HANDSHAKE_SIZE + usize::from(!self.handshake_started);
        let mut bytes_read = 0;
        while bytes_read < read_size {
            let n = self.read_input(read_size - bytes_read).await?;
            if n == 0 {
            ...
            self.sequence_number = self
                .sequence_number
                .wrapping_add(n.try_into().unwrap_or(u32::MAX));
            self.bytes_since_ack += n as u64;
        }

        let mut cursor = std::io::Cursor::new(self.read_buf.split().freeze());

        // Handshake output has a protocol-fixed size; do not hand the session
        // response buffer to the legacy Vec-only handshake API.
        let mut handshake_output = Vec::with_capacity(2 * handshake::RTMP_HANDSHAKE_SIZE + 1);
        handshaker.handshake(&mut cursor, &mut handshake_output)?;
    ...
    async fn drive(&mut self) -> Result<bool, crate::error::RtmpError> {
        ...
        if !self.reconnect_request_sent
            && self
                .caps_ex
                .is_some_and(|c| c.intersects(CapsExMask::Reconnect))
        ...
        if self.skip_read {
            ...
            let n = self.read_input(8192).await? as u32;
    ...
    async fn process_chunks(&mut self) -> Result<(), crate::error::RtmpError> {
        ...
        while let Some(chunk) = self.chunk_reader.read_chunk(&mut self.read_buf)? {
            ...
            let msg = MessageData::read_with_limits(&chunk, self.limits.amf)?;
            tokio::time::timeout(
                self.limits.handler_timeout,
                self.process_message(msg, msg_stream_id, timestamp),
            )
            .await
            .map_err(ServerSessionError::Timeout)??;
            processed += 1;
            // Bound response accumulation and make absolute deadlines observable
            // even when all bytes were already buffered.
            self.flush().await?;
            if processed >= self.limits.max_messages_per_turn {
    ...
    async fn process_message(
        ...
        if matches!(
            ...
            MessageData::AudioData { .. }
                ...
                | MessageData::DataAmf0 { .. }
        ) && !self.publishing_stream_ids.contains(&stream_id)
        {
            return Err(ServerSessionError::MediaBeforePublish.into());
        }
        match msg {
            MessageData::Amf0Command(command) => {
                self.on_command_message(stream_id, command).await?
            }
            MessageData::SetChunkSize(ProtocolControlMessageSetChunkSize { chunk_size }) => {
                self.on_set_chunk_size(chunk_size as usize)?;
            }
            MessageData::SetAcknowledgementWindowSize(
                ProtocolControlMessageWindowAcknowledgementSize {
            ...
            MessageData::DataAmf0 { data } => {
                self.handler
                    .on_data(stream_id, SessionData::Amf0 { timestamp, data })
                    .await?;
            }
            MessageData::Unknown(unknown_message) => {
                self.handler
                    .on_unknown_message(stream_id, unknown_message)
                    .await?;
            }
    ...
    async fn on_command_message(
        ...
        match command.command_type {
            CommandType::NetConnection(NetConnectionCommand::Connect(connect)) => {
                self.on_command_connect(stream_id, command.transaction_id, connect)
                    .await?;
            }
            CommandType::NetConnection(NetConnectionCommand::CreateStream) => {
                self.on_command_create_stream(stream_id, command.transaction_id)
                    .await?;
            }
            CommandType::NetStream(NetStreamCommand::Play { .. })
            | CommandType::NetStream(NetStreamCommand::Play2 { .. }) => {
                return Err(crate::error::RtmpError::Session(
            ...
            CommandType::Unknown(unknown_command) => {
                self.handler
                    .on_unknown_command(stream_id, unknown_command)
                    .await?;
            }
            // ignore everything else
            _ => {}
    ...
    fn on_set_chunk_size(&mut self, chunk_size: usize) -> Result<(), crate::error::RtmpError> {
        if self.chunk_reader.update_max_chunk_size(chunk_size) {
            ...
            Err(crate::error::RtmpError::Session(
                ServerSessionError::InvalidChunkSize(chunk_size),
            ))
        }
    }
    ...
    /// from the client.
    fn on_acknowledgement_window_size(
        &mut self,
        acknowledgement_window_size: u32,
    ) -> Result<(), crate::error::RtmpError> {
        if acknowledgement_window_size == 0 {
            return Err(ServerSessionError::InvalidAcknowledgementWindow.into());
        }
    ...
    async fn on_command_connect(
        ...
        if self.app_name.is_some() {
            return Err(ServerSessionError::InvalidPublish.into());
        }
        ProtocolControlMessageWindowAcknowledgementSize {
    ...
    async fn on_command_delete_stream(
        ...
        if !delete_stream_id.is_finite()
            || delete_stream_id.fract() != 0.0
            || delete_stream_id <= 0.0
            || delete_stream_id > u32::MAX as f64
        {
            return Err(ServerSessionError::InvalidPublish.into());
        }
    ...
    async fn on_command_publish(
        ...
        let Some(app_name) = &self.app_name else {
            ...
            return Err(crate::error::RtmpError::Session(
                ServerSessionError::PublishBeforeConnect,
            ));
        };
        if stream_id == 0 || self.publishing_stream_ids.contains(&stream_id) {
            return Err(ServerSessionError::InvalidPublish.into());
        }
        if self.publishing_stream_ids.len() >= self.limits.max_publishing_streams {
            return Err(ServerSessionError::ResourceLimit.into());
        }

        self.handler
            .on_publish(stream_id, app_name.as_ref(), publishing_name)
            .await?;

        self.publishing_stream_ids.push(stream_id);
        self.has_published = true;

        EventMessageStreamBegin { stream_id }.write(
            &self.chunk_writer,
            &mut BoundedWriter {
                buffer: &mut self.write_buf,
                limit: self.limits.max_write_buffer_bytes,
    ...
    async fn read_input(&mut self, max_bytes: usize) -> Result<usize, crate::error::RtmpError> {
        let available = self
            .limits
            .max_read_buffer_bytes
            .saturating_sub(self.read_buf.len());
        let count = available.min(max_bytes).min(8192);
        if count == 0 {
            return Err(ServerSessionError::ResourceLimit.into());
        }
        let mut data = [0; 8192];
        let n = tokio::time::timeout(self.limits.read_timeout, self.io.read(&mut data[..count]))
            .await
            .map_err(ServerSessionError::Timeout)??;
        if n > 0 {
            if self.read_buf.capacity() < self.read_buf.len() + n {
                let mut bounded = BytesMut::with_capacity(self.limits.max_read_buffer_bytes);
                bounded.extend_from_slice(&self.read_buf);
                self.read_buf = bounded;
            }
