...
use super::{
    RTMP_HANDSHAKE_SIZE, RtmpVersion, ServerHandshakeState, TIME_VERSION_LENGTH, current_time,
};
...
impl ComplexHandshakeServer {
    ...
    fn write_s1(&self, output: &mut Vec<u8>) -> Result<(), crate::error::RtmpError> {
        ...
        let data_digest =
            DigestProcessor::new(writer.into_inner().freeze(), RTMP_SERVER_KEY_FIRST_HALF);

        // We use the same schema version as the client and then write the result of the digest to the main writer.
        data_digest
            .generate_and_fill_digest(self.schema_version)?
            .write_to(output)?;
    ...
    fn write_s2(&self, output: &mut Vec<u8>) -> Result<(), crate::error::RtmpError> {
        ...
        let digest = data_digest.make_digest(
            &output[start..start + RTMP_HANDSHAKE_SIZE - RTMP_DIGEST_LENGTH],
            &[],
        )?;
