...
impl<'a> DigestProcessor<'a> {
    ...
    ) -> Result<DigestResult, ComplexHandshakeError> {
    ...
    fn split_message(
        &self,
        version: SchemaVersion,
    ) -> Result<(Bytes, Bytes, Bytes), ComplexHandshakeError> {
        ...
        let digest_data = self
            .data
            .slice(digest_offset..digest_offset + RTMP_DIGEST_LENGTH);
    ...
    pub fn make_digest(
        ...
        left: &[u8],
        right: &[u8],
    ) -> Result<[u8; 32], ComplexHandshakeError> {
    ...
    fn generate_and_validate(
        &self,
        version: SchemaVersion,
    ) -> Result<Bytes, ComplexHandshakeError> {
