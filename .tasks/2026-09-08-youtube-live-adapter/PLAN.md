...
- M4c Zweite Fix-Runde (Nachreview, zweiter Gate): finish holt bei offenem TransitionLive nie `transition live` nach (nur complete oder lokal schließen), Live nur bei passender Bindung (fehlende Bindung ist wiederaufnehmbarer Fehler), TransitionLive-Wiederholung im Abgleich nur bei aktivem Stream, prepare legt bei abgeschlossenem Broadcast im selben Aufruf einen neuen Run an, Kontrolle prüft privacyStatus, referenzen_setzen entfernt, Postgres-CAS-Tests für alle Schritt- und Schließ-Setter, eigene Fehlertexte für Rückzug und fremde Bindung, Vertrag um finish-Generationsausnahme und korrigierte Quota-Rechnung ergänzt.
