...
- Twitch: EventSub-WebSocket, Helix-Chatversand mit geprüftem `is_sent`, getrennte optionale Aktivitätsrechte, Tokenwiderruf, begrenzter Reconnect mit erlaubtem Twitch-Zielhost, Chat-Delete/Clear/Clear-User, Stream-Online/-Offline, Kanalpunkt-Add und -Update; Titel/Kategorie/Tags und Einlösungen über Helix.
...
Eingehende Twitch-Moderation hält die Chatansicht inzwischen konsistent: einzelne gelöschte Nachrichten, ein geleerter Chat und nach Timeout/Ban von Twitch geleerte Nachrichten eines Nutzers verschwinden aus dem Dock und bleiben auch bei EventSub-Replay entfernt. Eine allgemeine Moderationsoberfläche, die selbst Löschen, Timeout oder Ban auslöst, ist weiterhin nicht übernommen. Stream-Info-Verwaltung ist tatsächlich für Twitch vorhanden; Kick/YouTube-Titelverwaltung ist damit nicht freigegeben. TikTok meldet fehlende nachgewiesene Chatintegration ausdrücklich und erzeugt keine simulierten Nachrichten.
...
## Nachprüfung am 11. September 2026

Der Twitch-Pfad wurde gezielt erweitert und erneut lokal geprüft: 156 `uplink-chat`-Tests bestanden. Die EventSub-Session legt neben normalen Chat- und Aktivitätsabonnements jetzt auch `channel.chat.message_delete`, `channel.chat.clear_user_messages`, `channel.chat.clear`, `stream.online` und `stream.offline` an. Gelöschte bzw. von Twitch geleerte Chatzeilen werden im Dock als Tombstones gehalten, damit eine EventSub-Wiederholung oder ein verspätetes Duplikat die alten Nachrichten nicht erneut sichtbar macht. Die DOM-Suite umfasst jetzt 35 bestandene Regressionen einschließlich dieser Löschsemantik und der sichtbaren Stream-Online/-Offline-Aktivitäten. Diese lokale Nachprüfung verwendet synthetische Twitch-Nutzlasten und behauptet weiterhin keinen echten Account- oder Plattformnachweis.
