...
async fn destinations(
    ...
    for row in rows {
        ...
        let audio = crate::media_status::audio(sessions.first(), &platform);
        let requested = json!({"width":row.try_get::<_,Option<i32>>(3).map_err(|_|invalid())?,"height":row.try_get::<_,Option<i32>>(4).map_err(|_|invalid())?,"fps":row.try_get::<_,Option<i32>>(5).map_err(|_|invalid())?,"bitrate_kbps":row.try_get::<_,Option<i32>>(6).map_err(|_|invalid())?});
        let adjustment = crate::media_status::profile_reason(&platform, &requested, &active_profile);
        let reason = reason.map(str::to_owned).or(adjustment);
        outputs.push(json!({"platform":platform,"connection_generation":row.try_get::<_,i64>(7).map_err(|_|invalid())?,"rtmp_url":if blocked {""} else {endpoint.as_str()},"enabled":row.try_get::<_,bool>(2).map_err(|_|invalid())?,"blocked":blocked,"error":endpoint_error,"requested":requested,"active_profile":active_profile,"audio":audio,"output_state":output_state,"reason":reason,"publication_confirmed":false}));
    }
