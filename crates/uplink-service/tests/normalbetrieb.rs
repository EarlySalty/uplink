...
async fn normal_coordinator_keeps_healthy_output_and_reports_real_graph() {
    normal_audio_case(false, "youtube", false).await;
}

#[tokio::test]
#[ignore = "Benötigt isolierte PostgreSQL 16 und geprüften FFmpeg 8."]
async fn automatic_single_aac_reaches_twitch_and_vod() {
    normal_audio_case(true, "twitch", false).await;
}

#[tokio::test]
#[ignore = "Benötigt isolierte PostgreSQL 16 und geprüften FFmpeg 8."]
async fn automatic_two_aac_preserves_distinct_live_and_vod_feeds() {
    normal_audio_case(false, "twitch", false).await;
}

#[tokio::test]
#[ignore = "Benötigt isolierte PostgreSQL 16 und geprüften FFmpeg 8."]
async fn rejected_platform_publish_marks_the_entire_failed_session() {
    normal_audio_case(true, "twitch", true).await;
}

async fn normal_audio_case(single_audio: bool, healthy_platform: &str, all_failed: bool) {
    ...
    let result = std::panic::AssertUnwindSafe(async {
            ...
            loop {
                ...
                if healthy["output_state"]=="sending" {
                    ...
                    if healthy_platform == "twitch" {
                        assert_eq!(healthy["audio"],serde_json::json!({"source_tracks":if single_audio {1} else {2},"vod":if single_audio {"gleich"} else {"zweite_spur"}}));
                    }
        ...
        assert_eq!(audio.len(),if all_failed {0} else if single_audio || healthy_platform != "twitch" {1} else {2},"Nur Twitch bekommt eine zweite Tonspur");
        for wire in 0..if all_failed {0} else if single_audio || healthy_platform != "twitch" {1u8} else {2} {
        ...
        assert!(healthy["active_profile"].is_null());
        assert_eq!(healthy["audio"],serde_json::json!({"source_tracks":null,"vod":null}));
    }).catch_unwind();
