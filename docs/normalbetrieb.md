...
Twitch-Audio wird nicht mehr vom Nutzer umgeschaltet. Uplink erwartet für Twitch immer zwei verschiedene AAC-Mischungen: OBS-Ausgabespur 1 als Live-Mix und OBS-Ausgabespur 2 als VOD-Mix. Intern werden diese als Quell-Wire-Tracks 0 und 1 erkannt und für Twitch auf die Rollen Live und VOD geroutet. Sobald Twitch konfiguriert ist, lehnt die Konfigurationsprüfung einen anderen Live-/VOD-Track als `0/1` bereits beim Start ab. Das alte Datenbankfeld `twitch_audio_mode` bleibt während rollender Upgrades kompatibel lesbar, wird von neuen Schreibvorgängen und vom Mediengraph aber nicht mehr als Steuerung verwendet. Fehlt der VOD-Feed, hält ausschließlich der Twitch-Ausgang an; der Live-Mix wird niemals still als VOD-Ersatz kopiert. `effective_audio_mode=separate_vod`, `active_audio_mode` und `active_audio_routes` unterscheiden Sollregel und tatsächlich sendenden Graph; sie sind kein Plattform-VOD-Nachweis.
