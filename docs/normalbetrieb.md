...
Die ausdrücklich gespeicherte Twitch-Audiowahl ist `twitch_audio_mode=live|separate_vod`. NULL behält die vorhandene Instanzregel. Ausgelassene PUT-Felder erhalten die aktuelle Wahl auch bei konkurrierenden Änderungen. Live braucht nur den Live-Feed; separat verlangt einen anderen Feed. Fehlt dieser, hält ausschließlich der betroffene Ausgang an; der Live-Mix wird nie still als VOD-Mix eingesetzt. Die lokalen Normalbetriebsproben prüfen Ein-AAC-Live, getrennte Zwei-AAC-Ausgabe, fehlenden VOD-Feed neben gesundem Ziel sowie den Fall „Enhanced angefordert, aber nur sicherer Einzelstream ausführbar“ mit weiterhin getrennten Live-/VOD-AAC-Spuren. `active_audio_mode` beschreibt ausschließlich den lokal sendenden Graph. `active_audio_routes` legt zusätzlich die tatsächlich laufende Zuordnung `source_wire_track -> destination_wire_track` mit Rolle `live` bzw. `vod` offen. Beides ist ein lokaler Routingnachweis und ausdrücklich keine Bestätigung, dass Twitch den VOD-Mix im entstandenen Plattform-VOD verarbeitet hat.
